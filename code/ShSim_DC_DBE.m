function [TE, ULine, ULink, ILine, BRPower, iph, IS, mSlip, swFreq, ESPower, PropLoss] =...
    ShSim_DC_DBE(TE_demand, vehicle, speed, routedata, sim_config, limitation)
% ShSim_DC_DBE
% 0.1.1.2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%       ULine - Line voltage (V)
%       ULink - DC-link voltage (V)
%       ILine - Line current - total on train level (A)
%       BRPower - Brake resistor power - total on train level (W)
%       iMotor - Motor current (A) - per motor
%       mSlip - slip in the machine (-)
%       fMode - operation mode (not used)
%       ESPower - power used in the Energy Saver (W)
%       PropLoss - The actual loss in the propulsion system - total on train level (W)
%
% Ver       Date        Sign    Descr
% 0.1.1.1   2025-12-09  JR      New coding design (spaces) plus motor loss in Watts
% 0.1.1.2   2025-12-22  JR      'hasEfficiency' removed
%
ESPower = 0;
BRPower = 0;
iph = 0;
...
TE = ShSim_maxDBE(TE_demand, vehicle, speed, limitation);
if ZTot > 0
    p = -ULine / ZTot;
    q = TE * speed * 1000 / ZTot * 0.9;
    ILine = -p / 2 - sqrt(p^2 / 4 - q);
    ULink0 = ULine - ILine * ZTot;
else
    ULink0 = ULine;
    ULink = ULine;
    ILine = min(TE * speed * 1000 / ULink, -eps);
end
olddiff = inf;
...
while doCalc ~= 0
    ...
    switch vehicle.motorMethod  % 1 = fixed efficiency, 2 = motor map
        case 1 % fixed efficiency
            PInverter = tq * nMotorRPM * 2 * pi / 60;% / (1 + vehicle.TM_FixLoss);
            PInverter = PInverter + abs(PInverter * vehicle.TM_FixLoss);
            uph = min([speed / vehicle.vBasePwr, 1]) * ULink0 * sqrt(6) / pi;
            iph = -TE * speed * 1000 / (sqrt(3) * uph * 0.9)/vehicle.nAxDrive; % power factor 0.9 must be explained
    ...
    if isempty(vehicle.INV_FixLoss)
        ...
        if isempty(b)
            ...
            swFreq = freq * vehicle.nModesInvGenerator(b);
        end
    ...
    pLink0 = (pDcLink * vehicle.nHVSystems + vehicle.AuxPower / vehicle.APS_efficiency * 1000 + BRPower);
    ...
    if abs(1 - ULink / ULink0) > 0.005
        ...
        if doCalc
            ...
            if diff < 0.001
                doCalc = 0;
            elseif vehicle.OVCMethod % use brake resistor to maintain line current limits
                BRPower = max([BRPower + (iLineLim - ILine) * ULink, 0]);
            else
                if diff < olddiff
                    TE = ShSim_maxDBE(TE * iLineLim / ILine, vehicle, speed, limitation);
                else
                    TE = ShSim_maxTE(TE * (1 - diff * 0.5), vehicle, speed, limitation);
                end
                TE = max([TE, TE_demand]);
                olddiff = diff;
            end
