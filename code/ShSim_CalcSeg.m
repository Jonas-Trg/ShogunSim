function [r, stop_time, thermal_out, v_cut, d_cut] = ShSim_CalcSeg(trackdata,vehicle,sim_config, start_time, thermal_in, routedata, slopedata, data, hwb)
% ShSim_CalcSeg
% 0.1.1.0
%
...
% 34 MCM phase current
% 35 LCM phase current
% 
do_echo=data.echo;
if do_echo
...
[tvehicle,tRoute,error,op_mode]=check_parameters(routedata,routedata.track(section_index,9:end),vehicle,dT,do_echo,vehicle.class,sim_config);
mdone=error;
...
[TE_average,ULine,ULink,IL_new,BRPow,iMotor,IS,mSlip,fMode,ESPow,PropLoss]=ShSim_Efforts(0, tvehicle, V_new, tRoute, sim_config, limitation); 
...
if tvehicle.uFreq>0
    ...
    timedata(index,19) = thermal_in.iPhLCM_RMS^2;    %LCM phase current RMS
else
    ...
    timedata(index,19) = 0;                          %LCM phase current RMS
end
timedata(index,18) = thermal_in.iPhMCM_RMS^2;        %MCM phase current RMS
...
while (~mdone) && (time < sim_config.maxT+start_time)
    if toc>0.1
        ...
        if seconds(datetime-start_clock)>sim_config.time_out
            warndlg('Calculation interrupted by time-out','Time-out','modal')
            break
        end
        tic;
    end
    ...
    %Target speed
    v_target=min([routedata.track(section_index,2)/3.6 tvehicle.vMax tRoute.ATO_SpdMax/3.6])-tRoute.ATO_SpdMgn/3.6;
    ...
    %Calc traction
    [force_traction0,ULine,ULink,ILine0,BRPow,iMotor,IS,mSlip,fMode,ESPow,PropLoss]=ShSim_Efforts(F_target, tvehicle, V_old, tRoute, sim_config, limitation); %#ok<ASGLU>
...
%     IL_average=(IL_old + IL_new)/2;
    [TE_average,ULine,ULink,IL_average,BRPow,iMotor,IS,mSlip,fMode,ESPow,PropLoss]=ShSim_Efforts(TE_average, tvehicle, V_average, tRoute, sim_config, limitation);
...
%     IL_average=(T_start+T_end)/2*IL_old+(1-(T_start+T_end)/2)*IL_new;
    ...
    if V_new==0
        if TE_old==TE_new && (vehicle.notchMethod==0 || notch==vehicle.nNotchT)
            if do_echo
                errordlg(['Train is stuck at ' num2str(dist/1000) ' km'],'Warning','modal');
                fprintf(newline)
                cprintf('*err','Warning: ')
...
%     BRTemp = BRTemp + (vehicle.AmbientTemp - BRTemp)*(1-exp(-dt/TC)); %Brake resistor temperature
...
%     limitation.thermEffort=1;
    ...
    timedata(index,50) = op_mode;                       %allocate
...
% 30 Tunnel (effective C-factor of tunnel)
    ...
    if dist > endSection %We have reached the end of the section.
        ...
        if section_index < last_section_index %We have not yet reached the next station. From this point we need to go backwards.
            ...
            if (V_new > min1) && (min1<min2) %only if the speed limit in next section is lower we have already exceeded this, we need to go backwards
                ...
                [tempresult,index,time,ttime,tindex,limitation]=backtrack(timedata,section_index,index,routedata,trackdata,slopedata,vehicle,tvehicle.class,dT,do_echo,sim_config,min1,hwb,time,limitation,start_clock);
                if ~isempty(tempresult)
                    [timedata,limitation,v_cut,d_cut]=mergeReverseData(timedata(1:index,:),tempresult,v_target,dT,tvehicle,limitation,tRoute,sim_config,...
                        routedata.track(1,1),routedata.track(end,1),do_echo,hwb);
                    index=index+tindex;
                end
                clear tempresult;
                ...
                mdone=mdone || v_cut>0;
                Acc_new = 0;
            end %speed > current_section(2)/3.6
            [tvehicle,tRoute,error,op_mode]=check_parameters(routedata,routedata.track(section_index,9:end),vehicle,dT,do_echo,tvehicle.class,sim_config);
            mdone=mdone || error;
            ...
            [tempresult,index,time,ttime,tindex,limitation]=backtrack(timedata,section_index,index-1,routedata,trackdata,slopedata,vehicle,tvehicle.class,dT,do_echo,sim_config,0,hwb,time,limitation,start_clock);
            if ~isempty(tempresult)
                [timedata,limitation,v_cut,d_cut]=mergeReverseData(timedata(1:index,:),tempresult,v_target,dT,tvehicle,limitation,tRoute,sim_config,...
                    routedata.track(1,1),routedata.track(end,1),do_echo,hwb);
                index=index+tindex;
            ...
            V_new = 0;
            time = time - ttime - dT;
            [TE_average,ULine,ULink,IL_new,BRPow,iMotor,IS,mSlip,fMode,ESPow,PropLoss]=ShSim_Efforts(0, tvehicle, V_new, tRoute, sim_config, limitation); %#ok<ASGLU>
            ...
            timedata(index+1:index+n,10:12)=0; %statur current, modulation index
            ...
            for xx=index+1:index+n
                ...
                tcHigh=tcLow*tvehicle.BRThermTC_Tempfactor;
                TC=(BRTemp-tvehicle.AmbientTemp)/(tvehicle.BRTemp_High-tvehicle.AmbientTemp)*(tcHigh-tcLow) + tcLow;
                %                 BRThermC=tvehicle.BRThermC*tvehicle.nINV*tvehicle.nHVSystems*tvehicle.nUnits;
                BRTemp = BRTemp + (tvehicle.AmbientTemp - BRTemp)*(1-exp(-dT/TC));
                timedata(xx,9) = BRTemp; %Brake resistor temperature
                
                [timedata(xx-1:xx,:),limitation]=doThermal(timedata(xx-1:xx,:),IL_new,0,tvehicle,dT,limitation,1);
            end
    ...
    end %section_index < last_section_index
    time = time + dT;
end %(~mdone) && (time < simulation.maxT)
if time-start_time > sim_config.maxT
    errordlg(['Maximum running time of ' num2str(sim_config.maxT) ' seconds exceed']);
end
if ~exist('result','dir')
    mkdir('result');
end
...
thermal_out.iPhMCM_RMS    = timedata(end,18);    %MCM phase current RMS
thermal_out.iPhLCM_RMS    = timedata(end,19);    %LCM phase current RMS
...
[tvehicle,tRoute,error,op_mode]=check_parameters(routedata,routedata.track(backtrackSection,9:end),vehicle,dT,do_echo,class,sim_config); %#ok<ASGLU>
...
while V0_new < min([timedata(index,3) tvehicle.vMax])
    ...
    if 1 || force_resistance+F_slope>0
        v_target=max([tRoute.ATO_InitBrkSpd/3.6 V0_new])+tRoute.ATO_SpdMgn/3.6;
        v_target=min([routedata.track(backtrackSection,2)/3.6 tvehicle.vMax v_target tRoute.ATO_SpdMax/3.6])-tRoute.ATO_SpdMgn/3.6;
    else
    ...
    if dist1<routedata.track(backtrackSection,1)*1000
        ...
        [tvehicle,tRoute,error,op_mode]=check_parameters(routedata,routedata.track(backtrackSection,9:end),vehicle,dT,do_echo,tvehicle.class,sim_config); %#ok<ASGLU>
    end
    %Step up index
    tindex = tindex + 1;
    ...
    tempresult (tindex,50) = op_mode; %allocate
...
%         FAccLimit=FAccLimit+tvehicle.davies_b(tvehicle.LoadCondition)*speed*3.6+tvehicle.davies_c1(tvehicle.nUnits)*(speed*3.6)^2;
    case 3
        FAccLimit=FAccLimit+F_res + F_slope;
%         FAccLimit=FAccLimit+tvehicle.davies_a(tvehicle.LoadCondition);
%         FAccLimit=FAccLimit+tvehicle.davies_b(tvehicle.LoadCondition)*speed*3.6+tvehicle.davies_c1(tvehicle.nUnits)*(speed*3.6)^2 + F_slope;
end
...
% if tvehicle.nNotchT>0
%     notch=floor(F_target/min([tvehicle.maxTE tvehicle.maxPwrTEbase/speed])*tvehicle.nNotchT); %#ok<NASGU>
% end
...
%         F_target1=notch/tvehicle.nNotchT*min([tvehicle.maxTE tvehicle.maxPwrTEbase/speed]);
    else
    ...
    if F_target1>TE_old+maxStep
        F_target=TE_old+maxStep;
    elseif F_target1<TE_old-maxStep
        F_target=TE_old-maxStep;
    else
        F_target=F_target1;
    end
...
%     if abs(time-22.1)<0.06
%         fprintf('time     speed   acc   force    notch\n')
%     end
%     if time>=22.01 && time<30
%         fprintf([pad(num2str(time),6,'left')...
%                  pad(num2str(speed*3.6,'%5.2f'),8,'left')...
%                  pad(num2str(acceleration,'%5.2f'),8,'left')...
%                  pad(num2str(TE_old,'%5.2f'),8,'left')...
%                  pad(num2str([oldNotch notch],'%3.0f'),8,'left') newline])
%     end
    if notch>=0
...
function [tvehicle,troutedata,error,op_mode]=check_parameters(routedata,values,vehicle,delta_T,do_echo,class,sim_config)
tvehicle=vehicle;
    ...
    for ii=1:size(troutedata.parameters,2)
        ...
        if isfield(tvehicle,pName)
            if size(values,2)>=ii
                if ~isnan(values(ii))
                    if idx>0
%                         eval(['tvehicle.' char(pName) '(' num2str(idx) ')=' num2str(values(ii)) ';']);
        ...
        if strcmpi(char(troutedata.parameters(ii).name),'vehicle')
            if values(ii)>0
                if exist(fullfile(vehicle.path, char(troutedata.parameters(ii).vehicles(values(ii)))),'file')
                    ...
                    errordlg(['Failed to load vehicle file:' newline char(troutedata.parameters(ii).vehicles(values(ii))) newline 'Check file name and path'], 'File not found','modal');
                    tvehicle=vehicle;
                    error=1;
                end
            end
...
if any(tvehicle.LoadCondition == [1 2 3 4 5])
    ...
    tvehicle = ShSim_CalcMassRes(tvehicle);
%     tvehicle.davies_a = max(tvehicle.statmass * tvehicle.davies_a_dash,tvehicle.davies_a_actual)/1000;
%     tvehicle.davies_b = (tvehicle.davies_b1_dash*tvehicle.statmass + tvehicle.davies_b2)/1000;
else
    errordlg('Parameter ''LoadCondition'' must be an integer between 1..5', 'Illegal value','modal');
    error = 1;
end
...
limitation.thermPower=1; limitation.thermEffort=1;
...
% ta_end=(r(lnRD,3)-r(lnRD-1,3))/dt/vehicle.jerkrateT;
% ta_brk=(reversedata(1,3)-reversedata(2,3))/dt/vehicle.jerkrateB;
if lnRD>1 && size(reversedata,1)>1 %&& abs(1-speed/v_target)>0.01
    a_end=min([(r(lnRD,3)-r(lnRD-1,3))/dt r(lnRD,4)/vehicle.totMass]);
    ...
    if (a_end>vehicle.jerkrateT*dt || a_brk>vehicle.jerkrateB*dt) && (a_end+a_brk>(vehicle.jerkrateT+vehicle.jerkrateB)*dt) && (a_end>0)
        %     t1=a_end/vehicle.jerkrateT;
        %     t2=a_brk/vehicle.jerkrateB;
        %     dv1=a_end*t1-vehicle.jerkrateT*t1^2/2;
        %     dv2=a_brk-t2+vehicle.jerkrateB*t2^2/2;
        %
        x=lnRD;b=lnRD;
        d_cut=[0 inf];
        while r(b,2)<d_cut(2)
...
%25 slope
%     r(ii,26) = (r(ii,3) + r(ii-1,3))/2;   %26 average speed
    r(ii,27) = DBE_Average;   % 27 Average TE/DBE
    r(ii,28) = DBE_Average-Brk_average;   %28 Average FricBE
%     aa=(BE_ref0-BE_ref1)/vehicle.dDBE_jerkLimit;xx
%     if ii==lnRD+1
%         if isinf(vehicle.dDBE_jerkLimit)
%             r(ii,29) = 0;
%         else
%             r(ii,29) = (BE_ref0-TE_ref)/vehicle.dDBE_jerkLimit*ILine/2;
%         end
%     else
%         (BE_ref0-BE_ref1)/vehicle.dDBE_jerkLimit
%         (BE_ref0+BE_ref1)/(BE_ref1*2)*ILine
%         r(ii,29) = IL_Average;       % 29 Average line current
%     end
    r(ii,29)=IL_Average;
...
% timedata=[timedata(1:index,:);tempresult];
...
%Primary line current
    ...
    limitation.thermPower=min([1 limitation.IL_red]);
...
%     curve_resistance=tvehicle.curve_cr0/(abs(track(4))-tvehicle.curve_cr1)*tvehicle.statmass(tvehicle.LoadCondition);
